...
<body>
    ...
    <script>
        setTimeout(function(){{ window.close(); }}, 2000);
                ...
                {
                    await LoginWithGoogleCodeAsync(code, redirectUri);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task LoginWithGoogleCodeAsync(string code, string redirectUri)
        {
            try
            {
        ...
        private async Task OpenMainAppAfterLogin(string firebaseUid, string email, string displayName, string photoUrl = "")
        {

    ...
    public partial class register : Form
    {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                await OpenMainAppAfterRegister(
                    Convert.ToString(userCredential.User.Uid),
                    Convert.ToString(userCredential.User.Info.Email),
                    Convert.ToString(userCredential.User.Info.DisplayName),
                    Convert.ToString(userCredential.User.Info.PhotoUrl)
                );
            }
...
<body>
    ...
    <script>
        setTimeout(function(){{ window.close(); }}, 2500);
                ...
                {
                    await RegisterWithGoogleCodeAsync(code, redirectUri);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task RegisterWithGoogleCodeAsync(string code, string redirectUri)
        {
            try
            {
        ...
        private async Task OpenMainAppAfterRegister(string firebaseUid, string email, string displayName, string photoUrl = "")
        {
            var authUserService = new FirebaseAuthUserService(new global::FirestoreContext());

            var user = await authUserService.LoginOrCreateFirebaseUser(
                firebaseUid,
                email,
                displayName,
                photoUrl
            ...
            main.FormClosed += (s, e) =>
            {
                if (!main.IsRestarting && !this.IsDisposed)
                {
                    this.Close();
                }
            };
